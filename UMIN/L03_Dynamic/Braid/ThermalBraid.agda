...
module UMIN.L03_Dynamic.Braid.ThermalBraid where
...
module ThermalBraid (A : Type₀) (M : ℕ) (isA : isSet A) (tfd : TFDSpace A) (laws : KMSFlowLaws tfd) where
  ...
  -- section 3 で使う FinVec 局所作用（ここでは公理化してスコープを固定）
  postulate
    σFinVec' :
      (i : Fin (N ∸ 1)) (β : ℤ) → FinVec A N → (k : Fin N)
      → Dec (k ≡ weakenFin i) → Dec (k ≡ suc i) → A
    σFinVec :
      (i : Fin (N ∸ 1)) (β : ℤ) → FinVec A N → FinVec A N

  -- ================================================================
  -- 3. ブレイド関係式（Phase 1 の仕上げ：FarApartと局所可換性）
  -- ================================================================

  -- 因果分離を保証する位相的構造の種（シード）
  record FarApart (i j : Fin (N ∸ 1)) : Type₀ where
    constructor mkFarApart
    field
      i≢j : ¬ (weakenFin i ≡ weakenFin j)
      suc-i≢j : ¬ (suc i ≡ weakenFin j)
      i≢suc-j : ¬ (weakenFin i ≡ suc j)

  -- [導出補題] FarApart の i≢j から導出される条件
  -- （※ injSucFin 等を用いて証明を閉じてください）
  suc-i≢suc-j : ∀ {i j : Fin (N ∸ 1)} → FarApart i j → ¬ (suc i ≡ suc j)
  postulate
    suc-i≢suc-j-proof : ∀ {i j : Fin (N ∸ 1)} → FarApart i j → ¬ (suc i ≡ suc j)
  suc-i≢suc-j = suc-i≢suc-j-proof

  -- 点ごとの遠方可換性の証明（ネスト観測アプローチ）
  postulate
    far-commute-pointwise :
      ∀ (i j : Fin (N ∸ 1)) (β : ℤ)
      (far : FarApart i j)
      (f : FinVec A N) (k : Fin N)
      → σFinVec i β (σFinVec j β f) k
      ≡ σFinVec j β (σFinVec i β f) k

  -- ================================================================
  -- Theorem: 熱的ブレイド群の遠方可換性（巨視的な同一性への昇華）
  -- ================================================================
  postulate
    far-commute :
      ∀ (i j : Fin (N ∸ 1)) (β : ℤ) → FarApart i j
      → compEquiv (σ i β) (σ j β) ≡ compEquiv (σ j β) (σ i β)
