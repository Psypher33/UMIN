...
-- ※ 以前は §6 で +𝕜-reorder, -𝕜-sum を postulate していましたが、
--    現在はトレース線形性の仮定を上位レイヤ（E6Interface）に移し、
--    このモジュール自体は --safe でコンパイルできるようにしています。

module UMIN.L01_Arithmetic.FTS.AlbertAlgebra where
...
--    tr(X + Y) ≡ tr(X) + tr(Y)
--    tr(-X)    ≡ - tr(X)
--  は E6Interface 側で仮定（postulate）として扱うことにし、
--  ここでは 𝔍ᶜ の構造と演算の実装だけを提供します。
--  これにより、このモジュール自体は --safe を保ったまま、
--  上位レイヤから必要な線形性公理を差し込める設計になっています。
