...
-- ※ UMIN.L02_Obstruction.Ext1.UnifiedObstruction
-- ============================================================

module UMIN.L02_Obstruction.Ext1.UnifiedObstruction_v2 where
-- (以下、元のコードをそのまま維持)
...
kms-proof : ¬ ((x : Bool) → Path Bool true x)
...
record UnifiedObstruction : Type₁ where
  field
    ...
    ybe₃        : (a b c : Bool) → Path Bool
                  (swap₃ (swap₁ a b c) (swap₂ a b c) (swap₃ a b c)) c
    kms-holds   : ¬ ((x : Bool) → Path Bool true x)
...
minimal-unified = record
  ...
  ; ybe₃        = swap-inv₃
  ; kms-holds   = kms-proof
  ; coherence   = refl  -- witness = 1 = tor1-shadow
  }
...
alpha-from-unified u =
  cong (HermitianCore +_) (UnifiedObstruction.coherence u) ∙ alpha-arithmetic
...
-- EOF
-- postulate 数 : 0
-- メタ変数     : 0（swap-inv* / alpha-from-unified で解消）
-- ============================================================