...
module UMIN.L02_Obstruction.Ext1.ModalMagnitudeMonotonicity where
...
open import Cubical.Data.Nat.Order using (_≤_; ≤-trans; zero-≤; suc-≤-suc; ≤-refl; isProp≤)
...
record Adjunction (L R : Modality) : Type₁ where
  field
    ...
    -- 三角等式（zig-zag identity）: ε ∘ L(η) = id_{L A}
    -- L(η_A) はモナドとして bind L (λ x → η L (adj-unit x)) : L A → L R L A
    -- （bind L は (A → L B) を要求するため adj-unit をそのまま渡せない）
    adj-zig    : {A : Type₀} (a : act L A)
               → adj-counit (bind L (λ x → η L (adj-unit x)) a) ≡ a
      -- ⇒ ε ∘ Lη = id_L
    -- もう一方の三角等式（ηR ∘ ε 側）：embed-nat に必要
    adj-zag    : {A : Type₀} (a : A)
               → adj-counit (bind L (λ x → η L (η R x)) (η L a)) ≡ a
...
adjunction-to-leq L R adj = record
  { embed     = λ {A} la → η R (adj-counit adj (bind L (λ x → η L (η R x)) la))
  ; embed-nat = λ a →
      cong (η R) (adj-zag adj a)
  }
